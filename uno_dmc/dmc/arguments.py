...
parser = argparse.ArgumentParser(description='UNO-RL')

# General Settings
parser.add_argument('--xpid', default='uno_rl',
                    help='Experiment id (default: uno_rl)')
parser.add_argument('--save_interval', default=30, type=int,
                    help='Time interval (in minutes) at which to save the model')    
parser.add_argument('--objective', default='win', type=str, choices=['win'],
                    help='Reward objective (default: win)')    
...
# Simplification: we might run on CPU primarily if user resource constrained, 
# but defaulting to GPU logic if available.
parser.add_argument('--num_actor_devices', default=1, type=int,
                    help='The number of devices used for simulation')
parser.add_argument('--num_actors', default=10, type=int,
...
parser.add_argument('--savedir', default='uno_checkpoints',
                    help='Root dir where experiment data will be saved')

# Hyperparameters
parser.add_argument('--total_frames', default=100000000000, type=int,
                    help='Total environment frames to train for')
# Increase exploration initially? DouZero uses 0.01.
